<template>
  <div class="hello" id="subOutputRank-print">
  ...
  data () {
    return {
      ...
      contentedi_text:[
        ...
        "应注意减少刺激因素，尽量温水洁面，避免接触致敏源，减少肌肤裸露面积，注意防晒。同时注意营养平衡，要多吃牛奶、豆制品及新鲜的蔬菜和水果，避免吃鱼、虾、蟹等易引起过敏的食物。"
      ],
      contentedival:'',
      reportId:''
    }
...
//      alert(this.reportId)
      ...
      // 获取当前用户数据信息
       $.ajax({
        type:"get",
        url:"http://ws2.unescn.com/newProductRecommenderPageInit.json",
...
//      if(skinType!='' && scheme!='' && skinType!=null && scheme!=null && skinType!=undefined && scheme!=undefined){
      if(skinType!=''  && skinType!=null && skinType!=undefined){
          $.ajax({
            type:"post",
            url:"http://ws2.unescn.com/saveProductRecommender.json",
